...
class NEODataPreprocessor:
    ...
    def __init__(
        ...
        self.target_column = (
            target_column or
            config.get_param('preprocessing.target_column', 'hazardous')
        )
        self.test_size = (
            test_size if test_size is not None
            else config.get_param('preprocessing.test_size', 0.2)
        )
        self.random_state = (
            random_state if random_state is not None
            else config.get_param('preprocessing.random_state', 42)
        )
        self.stratify = (
            stratify if stratify is not None
            else config.get_param('preprocessing.stratify', True)
        )
