...
class NaiveBayesClassifier(BaseClassifier):
    ...
    def _build_model(self):
        ...
        self.model = GaussianNB(
            var_smoothing=self.params.get(
                'var_smoothing', default_params.get('var_smoothing', 1e-9))
        )
    ...
    def plot_distributions(self, feature_indices=None, feature_names=None):
        ...
        if self.model is None:
            print(
                "Error: Model has not been trained. Call train() before plotting distributions.")
            return
